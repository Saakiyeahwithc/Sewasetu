import { Bookmark, Building, Calendar, Building2, MapPin } from "lucide-react";
...
const JobCard = ({ job, onClick, onToggleSave, onApply, saved, hideApply }) => {
  ...
  return (
    <div
      className="bg-white rounded-2xl border border-gray-200 p-6 hover:shadow-xl hover:shadow-gray-200 transtition-all duration-300 group relative overflow-hidden cursor-pointer"
      onClick={onClick}
    >
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h3 className="font-semibold text-gray-900 text-base group-hover:text-emerald-600 transition-colors leading-snug">
              {job?.title}
            </h3>
        ...
        {user && (
          ...
          >
            <Bookmark
              className={`w-5 h-5 hover:text-emerald-600 ${
                job?.isSaved || saved ? "text-emerald-600" : "text-gray-400"
              }`}
            />
          </button>
        )}
