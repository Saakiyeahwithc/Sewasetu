  ...
  const onChangeStatus = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        //Update to local state after successful update
        setSelectedApplicant({ ...selectedApplicant, status: newStatus });
        toast.success("Application status updated successfully");
      }
    } catch (error) {
      console.error("Error updating status:", error);
      //Oprionally revert status if failed
        ...
        <div className="bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200">
            ...
            <button onClick={() => handleClose()} className="">
              <x className="p-2 hover:bg-gray-100 rounded-full transition-colors" />
            </button>
          </div>
          {/*Modal Content */}
          <div className="p-6">
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <p className="text-gray-700">{selectedApplicant.jop.title}</p>
              ...
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transiton-colors"
              >
